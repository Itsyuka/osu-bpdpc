...
class PerformanceCalculator {
  ...
  static use (difficultyCalculator) {
    if (difficultyCalculator.beatmap === null) throw new Error('You must have a valid difficulty calculator instance')
  ...
  calculate (score) {
    ...
    this.speedPerformance = Math.pow(5 * Math.max(1, this.speedDifficulty / 0.0675) - 4, 3) / 100000
  ...
  accuracyCalc (count300, count100, count50, countMiss) {
    ...
    let acc = 0
    if (totalHits > 0) {
      acc = Math.max(0, Math.min(1, (count50 * 50 + count100 * 100 + count300 * 300) / (totalHits * 300)))
    }
    return acc
  }
  ...
  get overallDifficulty () {
    ...
    let odms = Math.min(79.5, Math.max(19.5, (79.5 - Math.ceil(6 * od)))) / this.timeRate
    return (79.5 - odms) / 6
  }
  ...
  get approachRate () {
    ...
    let arms = Math.min(1800, Math.max(450, (ar <= 5 ? (1800 - 120 * ar) : (1200 - 150 * (ar - 5))))) / this.timeRate
