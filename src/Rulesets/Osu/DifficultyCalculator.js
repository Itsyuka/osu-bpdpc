...
class DifficultyCalculator {
  ...
  calculate () {
    ...
    this.starDifficulty = this.aimDifficulty + this.speedDifficulty + Math.abs(this.speedDifficulty - this.aimDifficulty) * 0.5
    return this
  }
  ...
  generateHitObjects () {
    ...
    for (let hitObject of this.beatmap.HitObjects) {
      ...
      if (previousHitObject) {
        this.calculateHitObjectStrain(difficultyHitObject, previousHitObject, 0)
        this.calculateHitObjectStrain(difficultyHitObject, previousHitObject, 1)
      }
    ...
    for (let hitObject of hitObjects) {
      while (hitObject.startTime > intervalEnd) {
        ...
        if (previousHitObject !== null) {
          maxStrain = previousHitObject.strains[difficultyType] *
            Math.pow([0.3, 0.15][difficultyType], (intervalEnd - previousHitObject.startTime) / 1000)
        }
        intervalEnd += 400
      }
  ...
  calculateHitObjectStrain (hitObject, previousHitObject, difficultyType) {
    ...
    if (!(hitObject.hitType & HitType.Spinner)) {
      ...
      res = this.calculateDistanceSpacingWeight(distance, difficultyType) * scaling
    }

    res /= Math.max(timeElapsed, 50)
    hitObject.strains[difficultyType] = previousHitObject.strains[difficultyType] * decay + res
  }
  ...
  get circleSize () {
    ...
    return Math.max(0, Math.min(10, this.beatmap.Difficulty.CircleSize * multiplier))
  }
